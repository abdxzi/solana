...
pub fn add_reaction(ctx: Context<AddReactionContext>, reaction: ReactionType) -> Result<()> {
    ...
    // Update tweet likes or dislikes based on reaction type
    match reaction {
        ReactionType::Like => {
            // Safely increment likes, handling overflow
            tweet.likes = tweet.likes.checked_add(1).ok_or(TwitterError::MaxLikesReached)?;
        }
        ReactionType::Dislike => {
            // Safely increment dislikes, handling overflow
    ...
    // Save the reaction value in tweet_reaction
    tweet_reaction.reaction = reaction;

    // Update other fields in the tweet_reaction account
